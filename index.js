...
console.log(products)
    ...
    categories.forEach(item => {
        let btn = `<button class="btn btn-primary  ctg-btn" onClick="ctgBtn(this)" value=${item}>${item}</button>`
        document.getElementById("categories").innerHTML += btn;
    })

    products.forEach(product => {
        let card = `<div data-aos="flip-down" class="card m-5 col-12 col-md-6 col-lg-4 col-xl-3" style="width: 18rem;">
...
const search = async() => {
    ...
    cardContainer.innerHTML ="";
    products.forEach(product => {
        let card = `<div class="card m-5 col-12 col-md-6 col-lg-4 col-xl-3" style="width: 18rem;">
        <img src=${product.thumbnail} class="card-img-top d-block mx-auto" alt="...">
        <div class="card-body">
          <h5 class="card-title">${product.title}</h5>
<p class="card-text">${product.description}</p>
